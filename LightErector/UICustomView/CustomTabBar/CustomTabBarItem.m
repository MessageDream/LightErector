...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self animateContentWithDuration:AnimationDuration];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGRect container = CGRectInset(rect, Padding, Padding);
    container.size.height -= TopMargin;
    container.origin.y += TopMargin;
    ...
    content.size.height = MIN(MAX(CGRectGetWidth(imageRect), CGRectGetHeight(imageRect)) + ((displayTabTitle) ? (Margin + CGRectGetHeight(labelRect)) : 0), CGRectGetHeight(container));
    ...
    imageContainer.size.height = CGRectGetHeight(content) - ((displayTabTitle) ? (Margin + CGRectGetHeight(labelRect)) : 0);
    ...
    CGFloat offsetY = rect.size.height - ((displayTabTitle) ? (Margin + CGRectGetHeight(labelRect)) : 0) + TopMargin;
    ...
    if (!self.selected) {
        ...
        CGContextSaveGState(ctx);
        {
            ...
            CGContextFillRect(ctx, CGRectMake(0, TopMargin, 1, rect.size.height - TopMargin));
        ...
        CGContextSaveGState(ctx);
        {
            [[UIColor colorWithPatternImage:[UIImage imageNamed:_selectedBackgroundImageName ? _selectedBackgroundImageName : @"AabBarController.bundle/noise-pattern"]] set];
